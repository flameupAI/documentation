...
<Callout kind="info">**Permissions**: writes (`/track`, `/track/batch`) need `events:write`; reads (`/people/{id}/events`, `/events/search`) need `events:read`. `/identify` is a people write — see that section.</Callout>
