...
person has. The fields below — content, targeting and template context — are
the complete set the endpoint honours.
...
with those `{{campaign.*}}`/`{{event.*}}` placeholders unresolved.</Callout>
