...
A public key *can* call `/identify`, but only if it holds `people:identify` —
a narrow scope that authorizes that one route and nothing else. It exists so an
app can set the traits it is the source of (locale, app version, notification
opt-in) without holding `people:write`, which would let it rewrite any person in
the workspace.

Send traits your backend owns — plan, entitlements, lifetime value — from your
backend. Those are the ones journeys branch on, and a client that could write
them could reroute any user it could name.
