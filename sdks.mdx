...
await FlameUp.initialize(FlameUpOptions(
  ...
  trackedEvents: {'login_successful', 'start_quiz', 'sales_page_viewed'},
  eventNamePrefix: 'tl_',
  enabled: true,
));
```
...
| `trackedEvents` | `Set<String>` | `{}` | Allowlist. Empty sends nothing |
| `trackedScreens` | `Set<String>` | `{}` | Same rule for screen views |
...
<Callout kind="tip">`trackedEvents` is an allowlist, not a filter you can skip. Most apps define hundreds of analytics events, every Flameup event re-evaluates that person's journeys server-side, and your workspace ingest budget is shared across everything. Add a name only once a campaign branches on it.</Callout>
