...
<Callout kind="info">`status` is serialized on **every** person read — Get, List,
...
consent state is set from the dashboard.</Callout>

<Callout kind="warning">**Trait keys are validated, and six are reserved.** A key
must start with a letter and contain only letters, digits and underscores
(`^[a-zA-Z][a-zA-Z0-9_]*$`), so dots, spaces, hyphens and leading digits are
...
just that record's entry.</Callout>
