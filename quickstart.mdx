...
// Send order confirmation (personId is the Flameup person ID from Step 2)
...
# Send order confirmation (person_id is the Flameup person ID from Step 2)
...
<Callout kind="info">The Push API requires the Flameup `person_id` (UUID returned when creating/identifying a user), not your external user ID.</Callout>
