...
<Callout kind="warning">Device registration requires the `devices:write` scope. The dashboard's key editor does not offer a Devices category yet, so a key created in the UI will get `403 Insufficient permissions` here — create a key with `devices:write` (or `devices:*`, or full access `*`) through the API keys endpoint, or skip ahead to Step 5.</Callout>
