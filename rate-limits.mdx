...
| `X-RateLimit-Scope` | Which budget was exhausted, e.g. `ws-ingest`, `edge`. Lets you tell "my workspace is over quota" from "the shared backstop is shedding load". |
...
`scope` names the budget you exhausted, matching `X-RateLimit-Scope`. `gate` is
`local` when your own limit was hit and `shared` when the process-wide backstop
was shedding load. Quote both if you contact support.
