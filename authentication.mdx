...
| **Devices** | `devices:register`, `devices:read`, `devices:write` | `devices:register` registers/unregisters only (safe to embed in a mobile app); `devices:read` lists only; `devices:write` does both. Grantable only when creating a key via the API — the dashboard editor has no Devices category yet |
