...
| **Devices** | `devices:read`, `devices:write` | Register and list push tokens. All device routes check `devices:write`, so `devices:read` currently grants nothing. Grantable only when creating a key via the API — the dashboard editor has no Devices category yet |
