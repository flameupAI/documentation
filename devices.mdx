...
<Callout kind="info">**Required Permission**: every device endpoint — including the read-only List Devices — is guarded by `devices:write`. The `devices:read` scope exists but currently grants access to nothing, so do not rely on it.</Callout>

<Callout kind="warning">Grant `devices:write` (or the `devices:*` wildcard, or full access `*`) when you create the key **via the API**. The dashboard's key editor does not yet show a Devices category, so keys created in the UI cannot get this scope — create the key through the [API keys endpoint](/authentication) instead.</Callout>
