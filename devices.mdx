...
<Callout kind="info">**Required Permission**: device operations are guarded by `devices:write`. There is currently no granular `devices` scope you can grant on its own — use a full-access (`*`) API key, or set a `devices:*` wildcard when creating the key via the API.</Callout>
